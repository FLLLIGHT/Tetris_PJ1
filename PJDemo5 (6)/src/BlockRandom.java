...
//the class is used to get random block and connect random block with the class Block
public class BlockRandom extends Block implements Serializable {

    //cells[][] is the cells with random rows and random columns and random blocks which you get first.
    protected boolean cells[][];

    //to avoid Array overflow, i add one row or column to every side of the cells[][], and get bigCells[][]
    protected boolean bigCells[][];

    //the copy of the cells, eliminate it to test how many different separate blocks are there in the cells, so that we can judge whether the cells is usable.
    protected boolean examine[][];

    //finalCells are the cells we get usable, without empty rows or columns.
    protected boolean finalCells[][];

    //newCells are based on the finalcells, expand finalcells to 3*3 or 4*4 or 5*5 ,so that the center of rotate is the center of the cells.
    protected boolean newCells[][];

    //the rows and columns of cells, finalcells and newcells.
    protected int rows;
    ...
    //用递归去消除一个方块和所有和他有连接的方块，以此类推，从而消除一整个连在一起的方块组，若只消除一次就把方块全部消除完，则所得的方块合法，超过一次则此方块不合法
    ...
    //judge whrther the row is empty
