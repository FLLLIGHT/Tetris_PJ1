...
public class Game {                                                   //待修复bug:cross模式跨越边界到底不一致时的判断，逐列判断
    ...
    private boolean collideCrossOneColumn(int nextPosRow, int nextPosColumn, boolean[][] nextBlockCells,int cellsColumn) {
        // todo
        for(int i=0;i<=currentBlock.getRows()-1;i++)
        {
                if(nextBlockCells[i][cellsColumn]&&cells[nextPosRow+i+1][Math.floorMod((nextPosColumn+cellsColumn),columns)])
                {
                    return true;
                }
